...
Security fixes are provided for the most recent CDexamSave release on the Moodle versions explicitly listed as supported in its public release record. Sites should also run a Moodle and PHP branch that still receives security updates.
...
Report privately to `carlosdiazbueno@gmail.com` with:

- CDexamSave, Moodle, PHP and database versions;
...
The documented limitation that client-side monitoring can be disabled or bypassed on an unmanaged device is not by itself a vulnerability. CDexamSave is not a locked browser and does not claim to provide a trusted client.
